...
def convert_schematic(asc_file: str, 
                     stroke_width: float = 1.0, dot_size_multiplier: float = 0.75,
                     scale: float = 0.1, font_size: float = 22.0, export_json: bool = False):
    """
    Convert an LTspice schematic to SVG format.
    
    Args:
        asc_file: Path to the .asc schematic file
        stroke_width: Width of lines in the SVG
        dot_size_multiplier: Size of junction dots relative to stroke width (default: 0.75)
        scale: Scale factor for coordinates (default: 0.1 = 10x scale down)
        font_size: Font size in pixels before scaling (default: 22.0)
        export_json: Whether to export intermediate JSON files (default: False)
    """
    ...
    print(f"Parsing schematic file: {asc_file}")
    ...
    for symbol in schematic_data['symbols']:
        ...
        if asy_file.exists():
            ...
            if export_json:
                ...
                print(f"Exported symbol data to: {symbol_json}")
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stroke-width", type=float, default=4.0,
                      help="Width of lines in the SVG")
    ...
    parser.add_argument("--font-size", type=float, default=22.0,
                      help="Font size in pixels before scaling (default: 22.0)")
